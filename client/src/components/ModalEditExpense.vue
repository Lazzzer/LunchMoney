    ...
    <div class="modal-new-budget w-screen h-screen absolute top-0 left-0 flex items-center justify-center ">    
      ...
      <div class="w-5/6 bg-lunchPurple-800 rounded-lunch border border-lunchPink-600 z-10" style="height: 400px;max-width:600px;">
        ...
        <div v-if="!expenseEdited && !sameExpense">
          ...
          <textarea v-model="newDescription" class="form-textarea mt-10 mt-1 block w-3/4 mx-auto bg-transparent border border-lunchPink-600 placeholder-lunchPurple-200 text-white focus:outline-none focus:border-white focus:shadow-none" rows="3" placeholder="Description of the expense"></textarea>
          <div @click="editExpense" class="no-highlight-color cursor-pointer w-3/5 mx-auto mt-6 block py-3 px-3 rounded-full bg-lunchPink-600 text-lunchPurple-700 text-center font-black uppercase text-lg focus:outline-none">SAVE CHANGE</div>
        </div>
      </div>
    </div>
  </div>
