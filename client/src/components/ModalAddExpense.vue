      ...
      <div class="z-10 w-5/6 border bg-lunchPurple-800 rounded-lunch border-lunchPink-600" style="height: 400px;max-width:600px;">
        ...
        <div v-else-if="noBudget" class="flex items-center w-full h-full">
          <div class="w-full">
            ...
            <h1 class="mx-4 mt-4 text-sm italic text-center text-red-500">No active budget found. <br>Make one before creating an expense.</h1>
            <div @click="$emit('closing-modal')" class="block w-3/5 px-3 py-3 mx-auto mt-4 text-lg font-black text-center uppercase rounded-full bg-lunchPink-600 text-lunchPurple-700 focus:outline-none focus:bg-lunchPink-700 hover:bg-lunchPink-700">EXIT</div>
          </div>
        </div>
        <div v-else>
          <h2 class="mt-2 ml-4 text-2xl italic font-black uppercase text-lunchPink-600">NEW EXPENSE</h2>
          ...
          <textarea v-model="description" class="block w-3/4 mx-auto mt-1 mt-10 text-white bg-transparent border form-textarea border-lunchPink-600 placeholder-lunchPurple-200 focus:outline-none focus:border-white focus:shadow-none" rows="3" placeholder="Description of the expense"></textarea>
          <div @click="createExpense" class="block w-3/5 px-3 py-3 mx-auto mt-6 text-lg font-black text-center uppercase rounded-full cursor-pointer no-highlight-color bg-lunchPink-600 text-lunchPurple-700 focus:outline-none">CREATE</div>
        </div>
      </div>
    </div>
  </div>
