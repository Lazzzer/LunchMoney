<template>
  <div class="mt-4 expenseTab border border-lunchPink-600 rounded-lunch" style="height:calc(100% - 194px);">
    <div @click="openModalAdd = true" class="cursor-pointer no-highlight-color flex justify-center w-17/20 mx-auto mt-5 block py-3 px-3 rounded-full bg-lunchPurple-900 text-lunchPurple-700 text-center font-black uppercase text-lg focus:outline-none focus:bg-lunchPink-700">
    ...
    <div v-else class="w-17/20 mx-auto overflow-y-scroll" style="height:calc(100% - 130px);">
      <div v-for="(expense, index) in expenses" :key="`expense-${index}`" 
           ...
           :id="index"
           class="w-full bg-lunchPurple-500 mb-2 rounded-expenses h-13 flex items-center relative"
      >
