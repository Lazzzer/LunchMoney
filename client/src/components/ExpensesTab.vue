<template>
  <div class="mt-4 border expenseTab border-lunchPink-600 rounded-lunch" style="height:calc(100% - 194px);">
    <div @click="openModalAdd = true" class="flex justify-center block px-3 py-3 mx-auto mt-5 text-lg font-black text-center uppercase rounded-full cursor-pointer no-highlight-color w-17/20 bg-lunchPurple-900 focus:outline-none">
      ...
      <h2 class="ml-5 text-base italic font-black uppercase text-lunchPink-600">CREATE NEW EXPENSE</h2>
    </div>
    <h2 class="mx-auto my-2 text-base italic font-black uppercase w-17/20 text-lunchPink-600">LATEST EXPENSES</h2>
    <div v-if="noExpenses" class="flex flex-col items-center justify-center mx-auto w-17/20 rounded-lunch bg-lunchPurple-500" style="height:calc(100% - 130px);">
    ...
    <div v-else class="mx-auto overflow-y-scroll w-17/20" style="height:calc(100% - 130px);">
      <div v-for="(expense, index) in expenses" :key="`expense-${index}`" 
           v-touch:swipe.left="swipe(index)"
           v-touch:swipe.right="swipe(index)"
           :id="index"
           class="relative flex items-center w-full mb-2 bg-lunchPurple-500 rounded-expenses h-13"
      >
